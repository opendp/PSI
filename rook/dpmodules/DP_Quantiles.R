...
# quantiles. This is done by outputting a monotone approximate CDF of
...
computeInterval <- function(tree, val, range, gran) {
...
partialMeans <- function(vector, i, j) {
	# Helper function that computes the mean of the values in a subset
	# of a vector
	
	return(sum(vector[i:j])/(j - i + 1))
}

smoothVector <- function(vector) {
	# Helper function that computes the best monotone approximation 
	# to a vector
	#
	# Args:
	# vector: the vector we wish to make monotone
	#
	# Returns: a vector that is the best monotone approximation
	#
	returnVector <- rep(0, times = length(vector))
	mean_array <- array(0, dim = c(length(vector), length(vector)))
	for(i in 1:length(vector)) {
		for(j in i:length(vector)) {
			mean_array[i,j] <- partialMeans(vector, i, j)
		}
	}
	
	for(k in 1:length(vector)) {
		temp_vector <- rep(0, times = length(vector) - k + 1)
		for(j in k:length(vector)) {
			sub_vector <- mean_array[1:j, j]
			temp_vector[j - k + 1] <- max(sub_vector)
	    }
	    returnVector[k] <- min(temp_vector)
	}
	
	return(returnVector)
}

computeCDF <- function(eps, cdfstep, data, range, gran) {
  # Releases an approximate CDF of a data set in a DP manner. Returns these values in a vector that is monotone
  #
  # Args: 
  ...
  #
  # Returns:
  #  A vector whose values are the approximate counts for
  ...
  for(i in 1:(2^tree_depth - 1 + universe_size)) {
  	...
  	#JM 12/2014 changed this to return tree for TwoRavens rather than CDF
  '
  ...
  return(smoothVector(returnValue))
  '
  return(smoothVector(tree)) #JM do we want to call smooth vector here?
}

getAccuracy <- function(eps, beta, range, gran) {
  # Args: 
  #  eps: epsilon value for DP
  ...
  #
  #  The accuracy is interpreted as follows: The alpha value returned means that with probability 1 - beta, for any t with min <= t <= max, the algorithm's estimate of the count in [min, t] is within alpha of the true value
...
getParameters <- function(alpha, beta, range, gran) {
  # Args:
