...
printEstimate <- function(metadata) {
...
printStatistic <- function(statistic, name) {
  ...
  if (length(instances) == 1L) {
    cat('\n### Statistic: ', name, '\n', sep = "")
    printEstimate(statistic[[instances[[1]]]])
  }
  else {
    for(instance in instances) {
      cat('\n### Statistic: ', instance, '\n', sep = "")
      printEstimate(statistic[[instance]])
    }
  }
}
...
for (variable in names(metadata$variables)) {
  cat('\n## ', variable, '\n', sep = "")
  printVariable(metadata$variables[[variable]])
}
```
